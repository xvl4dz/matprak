...
int main() {
    ...
    double dt = 0.05, T = 60.0, R = 1.5;
    double omega = (6.0 * M_PI) / T; 
    ...
    for (double t = 0; t <= T + 1e-8; t += dt) {
        f->xc = R * std::cos(omega * t);
        f->yc = R * std::sin(omega * t);
        
        double base_A = (std::fmod(t, 10.0) < 5.0) ? 1500.0 : 0.0;
        f->A = base_A * std::min(1.0, t / 0.5);
        ...
        *u0 = u;
        ...
        write_vtu(mesh, u, "step5_combined_" + std::to_string(step) + ".vtu");
    }
    return 0;
}
